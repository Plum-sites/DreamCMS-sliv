				...
				<v-list-tile avatar :key="mail.id" @click="" ripple>
					...
					<v-list-tile-content>
						<div v-if="mail.type === 'inbox'">
							...
							<span class="grey--text fs-12 fw-normal">{{mail.from.email}}</span>
						</div>
						<template v-else>
							<h5 class="mb-0">{{mail.to.name}}</h5>
							<span class="grey--text fs-12 fw-normal">{{mail.to.email}}</span>
						</template>
						<span class="fs-12 fw-normal"><strong>Subject:</strong> {{mail.subject}}</span>
					</v-list-tile-content>
               <v-list-tile-action>
                  <v-list-tile-action-text>
                     <span class="grey--text fs-12 mb-2 d-block text-xs-right fw-normal">{{mail.date}}</span>
